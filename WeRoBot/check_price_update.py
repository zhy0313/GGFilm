...
def parse_price_sim(url, fff, new_driver):
    try:
        ...
        try:
            ...
            username_input_box.send_keys(u'夜空下的邮差')
            ...
            password_input_box.send_keys('summychou0808')
...
if __name__ == '__main__':
    ...
    all_items = ItemStorage.objects.all()
    ...
    if len(changed_list) != 0:
        ...
        sheet.write(0, 1, u"商品描述")
        sheet.write(0, 2, u"商品价格")
        sheet.write(0, 3, u"商品淘宝价格")
        sheet.write(0, 4, u"店铺淘宝地址")
        for (j, item) in enumerate(changed_list):
            sheet.write(j+1, 0, item.ItemShopName)
            sheet.write(j+1, 1, item.ItemTitle)
            sheet.write(j+1, 2, item.ItemPrice)
            sheet.write(j+1, 3, item.ItemTaoBaoPrice)
            sheet.write(j+1, 4, item.ItemLink)
        excel.save("excels/PriceTrace-{0}.xls".format(current_time))

        msg.attach_file("excels/PriceTrace-{0}.xls".format(current_time))
        msg.send()
